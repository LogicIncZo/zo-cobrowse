...
- **`lib/write-assist.js`** (pure, no chrome/DOM deps):
  - `isEnhanceableField(info)` → boolean — textarea (not disabled/readonly) or contenteditable rich editor.
  - `buildEnhancePrompt({ text, instruction, field, page, acceptsMarkdown })` → string. Rules baked in: keep the user's voice and first-person point of view; expand the lead into a complete answer; **do not invent specific facts not implied by the lead** (honest-copy); **no tools/commands/search** (field-scoped one-shot — live-observed: Zo otherwise turns it into an agent turn that reads workspace files); **tag protocol** — final text between `<write-assist>` tags, nothing outside; **Markdown branch** — contenteditable editors accept Markdown (headings/lists/bold welcome), plain textareas forbid it (round 3, user-reported from a live GitHub-issue enhance); respect `maxLength` when present. Field context = label + placeholder + markdown flag; page context = URL + title only (token-cheap, no DOM text).
  - `parseEnhanceResponse(raw)` → `{ text }` — keeps ONLY the `<write-assist>` tag content when present (anything outside is Zo's intermediate narration — "Let me quickly ground this…" warm-ups, tool summaries — dropped); untagged replies fall back to trim + strip a wrapping code fence and one wrapping quote pair.
