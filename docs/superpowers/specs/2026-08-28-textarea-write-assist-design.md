...
| Icon visibility | Delegated `focusin`/`focusout` on `document`; icon shows for the focused eligible textarea, hidden on blur with a ~150ms grace so the icon itself is clickable; repositioned on scroll/resize while active | First focus tracking in the repo. Focus-based (not hover) avoids flicker; one icon instance moves between fields. |
...
- `contenteditable` rich-text editors (zero support exists anywhere today; a separate, harder ticket — many are framework widgets with their own mutation model).
