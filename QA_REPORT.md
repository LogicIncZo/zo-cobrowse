...
> **2026-09-22 — v0.3.4.3 !handoff bug bash:** second 0.3.4 stabilization round, over the Lane E delegate loop (`tests/test-prompts/probe-handoff-bash.ts` + a standalone-process sweep test). Five fixes, one zo-loop cycle each: #368 prose-only turns stranded runs (blocked at the stream choke; awaited priming flip), #369 resume-path tier-0 crawl (force-attach, #351 class), #370 double-run per chat (compose guard mirrored), #371 Jev click machinery on readonly runs (prompt + executor, run-scoped), #372 stale badge after SW restart. All gates green; findings queue empty. Process note: item 370's machine record reached done before its PR merged (a dev conflict surfaced at merge time) — resolved and the merged head's CI recorded on the PR; later items record merge-approval only after the merge succeeds.

> **2026-09-22 — v0.3.4.2 prompt-efficiency bug bash:** owner-directed audit of all `/zo/ask` prompt surfaces (`tests/test-prompts/probe-bloat-0342.ts`: 29 scenarios, per-section tokens). Four fixes, one zo-loop cycle each: #355 Jev block on read-downgraded turns (wrong gate), #356 visual mode's ~1.3k dead DOM tokens (`domSections` knob), #357 Jev block wording 148→123 (guardrails pinned), #358 triple-inlined workspace-write prompt deduped (anti-drift tested). Evals cache refreshed live post-#356 (24/24). All gates green; findings queue empty.
